...
namespace  NumberGUI
{
	...
	json Data::LoadDataJson(bool useBinary_) {
		...
		this->num_oneOfImgSizeL = j["num_oneOfImgSizeL"];
		this->num_filePathL = j["num_filePathL"];
		//----------------------------------
	...
	bool  Resource::Initialize()
	{
		...
		//★データ初期化
		//数字
		this->numImgs = ML::PartImage::Create(
		...
		this->numImgsL = ML::PartImage::Create(
			this->data->num_filePathL		//画像ファイルパス
			, this->data->num_imagesNumAll		//分割総数
			, (int)this->data->num_imagesNumSize.w	//横の分割数
			, (int)this->data->num_imagesNumSize.h	//縦の分割数
			, (int)this->data->num_oneOfImgSizeL.w		//分割した画像の横幅
			, (int)this->data->num_oneOfImgSizeL.h		//分割した画像の縦幅
		);
		return true;
	}
	//-------------------------------------------------------------------
	...
	bool  Object::Initialize()
	{
		...
		this->drawImage = this->res->numImgs;
		//★タスクの生成
	...
	//関数定義
	//大画像を使うか
	void Object::UseLarge(bool useFlag_) {
		if (useFlag_) {
			this->drawImage = this->res->numImgsL;
		}
		else {
			this->drawImage = this->res->numImgs;
		}
	...
	ML::Vec2 Object::DrawNumber(const ML::Vec2& pos_, int num_, bool useSign_, int digit_,  float alpha_) {
		...
		//1桁目（一の位）の左上座標を計算
		ML::Size2 imgSize(
			(float)this->drawImage->get_xSize()
			, (float)this->drawImage->get_ySize()
		);
		...
		for (int i = 0; i < digit; ++i) {
			this->drawImage->Draw(numDrawPos, drawNum % 10, true, alpha_);
		...
		if (useSign_) {
			if (num_ > 0) {
				this->drawImage->Draw(numDrawPos, 10, true, alpha_);
			}
			else if (num_ < 0) {
				this->drawImage->Draw(numDrawPos, 11, true, alpha_);
			}
		}
