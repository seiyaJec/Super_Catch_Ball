...
{
	//タスクに割り当てるグループ名と固有名
	const  string  defGroupName("シーン遷移生成");	//グループ名
	...
	public:
		...
		//データ変数は以下に追加する
		//逆ピラミッド
		int RPyramidCuts;		//分割数
		int RPyramidDistance;	//間隔
		int RPyramidMoveSpeed;	//移動速度

		int circleScaleUpSpeed;	//拡大速度
	...
	public:
		...
		enum TransitionsNum{ReversePyramid, CircleScaleUp};

		//シーン遷移生成
		//代入先もweak_ptrでの使用を推奨
		SceneTransition::SceneTransitionStd::WP Generate(int transitionsNum_, ML::Color color_ = ML::Color());
	};
}

using ScTransitionsNum = SceneTransitionGenerator::Object::TransitionsNum;