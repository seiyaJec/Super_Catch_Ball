...
#include  "Task_ResultFont.h"

namespace  ResultFont
{
	//＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
	// ゲームデータ
	//＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
	...
	json Data::LoadDataJson(bool useBinary_) {
		...
		if (useBinary_ == false) {
			//ファイルパス
			std::ifstream fin("./data/gamedata/resultFont.json");
			if (!fin) { return json(); }
			//JSONファイル読み込み
			fin >> j;
			//ファイル読み込み終了
		...
		this->pos = j["position"];
		this->imgFilePath = j["imageFilePath"];
		this->imgsize = j["imageSize"];
		this->renderPriority = j["renderPriority"];
	...
	bool  Resource::Initialize()
	{
		...
		//★データ初期化
		this->img = ML::Image::Create(this->data->imgFilePath, this->data->imgsize);
	...
	bool  Object::Initialize()
	{
		...
		//★データ初期化
		this->render2D_Priority[1] = this->res->data->renderPriority;
