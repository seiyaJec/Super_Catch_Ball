...
namespace  ResultManager
{
	...
	json Data::LoadDataJson(bool useBinary_) {
		...
		if (useBinary_ == false) {
			//ファイルパス
			std::ifstream fin("./data/gamedata/resultManager.json");
			if (!fin) { return json(); }
			//JSONファイル読み込み
			fin >> j;
			//ファイル読み込み終了
		...
		//----------------------------------
		//★データ読み込み処理
		this->waitDrawScoreTime = j["waitDrawScoreTime"];
		this->waitGenerateEffectTime = j["waitGenerateEffectTime"];
		this->effectPos = j["effectPos"];
	...
	bool  Object::Initialize()
	{
		...
		//★データ初期化
		ge->qa_numberGUI->UseLarge(true);
		ge->qa_scoreJudge->setDraw(false);
		ge->qa_scoreJudge->SetPos(this->res->data->scorePos);
		this->timeCnt = 0;
		this->loadingFlag = false;
		//★タスクの生成
		ResultBGImage::Object::Create(true);
		ResultFont::Object::Create(true);
		return  true;
	}
	//-------------------------------------------------------------------
	//「終了」タスク消滅時に１回だけ行う処理
	bool  Object::Finalize()
	{
		...
		if (!ge->QuitFlag() && this->nextTaskCreate) {
			//★引き継ぎタスクの生成
			TitleManager::Object::Create(true);
		}
	...
	void  Object::UpDate()
	{
		...
		//シーン遷移中の処理
			...
			auto sctr = ge->sceneTransition.lock();
			if (sctr) {
				if (sctr->CheckFinishAppear() == true) {
					//自身をキルしてシーン遷移を消滅させる
					this->Kill();
					sctr->Disappear();
				}
		...
		//エフェクト生成
		if (this->timeCnt == this->res->data->waitGenerateEffectTime) {
			ge->effectGenerator->GenerateEffect(EffectsNum::GorgeousBall, this->res->data->effectPos);
		}

		//スコア描画
		if (this->timeCnt == this->res->data->waitDrawScoreTime) {
			ge->qa_scoreJudge->setDraw(true);
		}
