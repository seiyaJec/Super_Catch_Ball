...
namespace ML {
	...
	Vec2& Vec2::operator*=(float value_) {
		*this = *this * value_;
		return *this;
	...
	Vec3::Vec3(float x_, float y_, float z_) :
		...
		z(z_)
	{}
	Vec3::Vec3(const DxLib::VECTOR& vec_) :
		Vec2(vec_.x, vec_.y),
		z(vec_.z)
	{}
	//ーーーーーーーーーーーーーーーーーーーーーーーーー
	...
	//ーーーーーーーーーーーーーーーーーーーーーーーーー
	//XYZ同値のオブジェクトを生成
	Vec3 Vec3::CreateXYZ(float XYZ_) {
		return Vec3(XYZ_, XYZ_, XYZ_);
	}
	//ーーーーーーーーーーーーーーーーーーーーーーーーー
	//jsonファイルへの書き出し
...
//	}
	...
	//ーーーーーーーーーーーーーーーーーーーーーーーーー
	//中心を原点にして変形描画
	void Image::DrawTransform(Vec2 pos_, const Transform& trs_, bool TransFlag, float alpha_) {
		SetDrawBlendMode(DX_BLENDMODE_ALPHA, alpha_ * 255);
		DrawRotaGraph3(
			(int)pos_.x,								//中心原点の座標x
			(int)pos_.y,								//中心原点の座標y
	...
	//ーーーーーーーーーーーーーーーーーーーーーーーーー
	//中心を原点にして変形描画
	void Image::DrawPartRect(ML::Vec2 pos_, const ML::Box2D& src_, bool transFlag_, float alpha_, bool flipFlag_) {
		SetDrawBlendMode(DX_BLENDMODE_ALPHA, alpha_ * 255);
		DxLib::DrawRectGraph(
			(int)pos_.x, (int)pos_.y,								//左上座標
			(int)src_.x, (int)src_.y, (int)src_.w, (int)src_.h,		//描画する部分の指定
			this->handle,											//基となる画像ハンドル
			transFlag_,												//透明度の有効/無効
			flipFlag_												//反転するか
		);
		SetDrawBlendMode(DX_BLENDMODE_NOBLEND, 0);
	}
	...
	bool Model3D::CheckAnimAttach(int animIndex_) {
		if (this->handleAnim.find(animIndex_) == this->handleAnim.end()) {
			return false;
		}
		else {
			return true;
	...
	//アニメーション
	//指定番号のアニメーションをアタッチ
	bool Model3D::AttachAnim(int animIndex_) {
		pair<int, int> animData;
		animData.first = animIndex_;
		animData.second = DxLib::MV1AttachAnim(this->handle, animIndex_);
		if (animData.second == -1) {
			return false;
		}
		this->handleAnim.insert(animData);
		return true;
	}
	//ーーーーーーーーーーーーーーーーーーーーーーーーー
	//指定番号のアニメーションをデタッチ
	bool Model3D::DetachAnim(int animIndex_) {
		//アニメーションが存在するか
		if(this->CheckAnimAttach(animIndex_) == false){
			return false;
		}

		DxLib::MV1DetachAnim(this->handle, this->handleAnim.at(animIndex_));
		this->handleAnim.erase(animIndex_);
		return true;
	}
	//ーーーーーーーーーーーーーーーーーーーーーーーーー
	//全てのアニメーションをデタッチ
	void Model3D::DetachAnimAll() {
		if (handleAnim.size() <= 0) {
			return;
		}

		for (auto& handle : this->handleAnim) {
			DxLib::MV1DetachAnim(this->handle, handle.second);
		}
		this->handleAnim.clear();
	}
	//ーーーーーーーーーーーーーーーーーーーーーーーーー
	//アニメーションの時間指定
	bool Model3D::SetAnimTime(float time_) {
		if (handleAnim.size() <= 0) {
			return false;
		}
		DxLib::MV1SetAttachAnimTime(this->handle, (*this->handleAnim.begin()).second, time_);
		return true;
	}
	//ーーーーーーーーーーーーーーーーーーーーーーーーー
	//アタッチ中のアニメーション総時間を受け取る
	float Model3D::GetAttachAnimTotalTime() {
		if (handleAnim.size() <= 0) {
			return 0.0f;
		}
		
		float total = 0;
		for (auto& data : this->handleAnim) {
			total += DxLib::MV1GetAttachAnimTotalTime(this->handle, data.second);
		}
		return total;
	}
	//ーーーーーーーーーーーーーーーーーーーーーーーーー
	...
	//ーーーーーーーーーーーーーーーーーーーーーーーーー
	//ファイルパスのゲッタ
	string Model3DClone::get_filePath() {
		//元のクラスが無ければ行わない
		if (wp_base.expired()) {
			return "none";
		}
		return Model3D::get_filePath();
	}
	//ーーーーーーーーーーーーーーーーーーーーーーーーー
	//画像ハンドルのゲッタ
	int Model3DClone::get_handle() {
		if (wp_base.expired()) {
			return -1;
		}
		return Model3D::get_handle();
	}
	//ーーーーーーーーーーーーーーーーーーーーーーーーー
	//アニメーション
	//指定番号のアニメーションをアタッチ
	bool Model3DClone::AttachAnim(int animIndex_) {
		//元のクラスが無ければ行わない
		if (wp_base.expired()) {
			return false;
		}
		Model3D::AttachAnim(animIndex_);
	}
	//ーーーーーーーーーーーーーーーーーーーーーーーーー
	//指定番号のアニメーションをデタッチ
	bool Model3DClone::DetachAnim(int animIndex_) {
		//元のクラスが無ければ行わない
		if (wp_base.expired()) {
			return false;
		}
		Model3D::DetachAnim(animIndex_);
	}
	//ーーーーーーーーーーーーーーーーーーーーーーーーー
	//全てのアニメーションをデタッチ
	void Model3DClone::DetachAnimAll() {
		//元のクラスが無ければ行わない
		if (wp_base.expired()) {
			return;
		}
		Model3D::DetachAnimAll();
	}
	//ーーーーーーーーーーーーーーーーーーーーーーーーー
	//アニメーションの時間指定
	bool Model3DClone::SetAnimTime(float time_) {
		//元のクラスが無ければ行わない
		if (wp_base.expired()) {
			return false;
		}
		Model3D::SetAnimTime(time_);
	}
	//ーーーーーーーーーーーーーーーーーーーーーーーーー
	//アタッチ中のアニメーション総時間を受け取る
	float Model3DClone::GetAttachAnimTotalTime() {
		//元のクラスが無ければ行わない
		if (wp_base.expired()) {
			return 0.0f;
		}
		return Model3D::GetAttachAnimTotalTime();
	}
	//ーーーーーーーーーーーーーーーーーーーーーーーーー
	//描画
	...
	//ーーーーーーーーーーーーーーーーーーーーーーーーー
	//コンストラクタ
	EffekseerInitData::EffekseerInitData()
	{}
	//ーーーーーーーーーーーーーーーーーーーーーーーーー
	//値設定
	void EffekseerInitData::Set(const string& filePath_, float scale_) {
		this->filePath = filePath_;
		this->scale = scale_;
	}
	//ーーーーーーーーーーーーーーーーーーーーーーーーー
	//jsonファイルへの書き出し
	void to_json(json& j_, const EffekseerInitData& efc_) {
		j_ = json{
			{"filePath", efc_.filePath},
			{"scale", efc_.scale},
		};
	}
	//ーーーーーーーーーーーーーーーーーーーーーーーーー
	//jsonファイル空の読み込み
	void from_json(const json& j_, EffekseerInitData& efc_) {
		j_.at("filePath").get_to(efc_.filePath);
		j_.at("scale").get_to(efc_.scale);
	}
	//ーーーーーーーーーーーーーーーーーーーーーーーーー
	//デバッグ情報
	string EffekseerInitData::DebugPrint() const {
		stringstream ss;
		ss << "filePath：" << this->filePath << '/';
		ss << "initPos：" << this->scale;
		return ss.str();
	}
	//ーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーー
	//エフェクシアオブジェクトクラス
	//ーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーー
	//コンストラクタ
	EffekseerObj::EffekseerObj(const EffekseerInitData& efkData_) :
		handleResource(LoadEffekseerEffect(efkData_.filePath.c_str(), efkData_.scale))
	{
	}
	//ーーーーーーーーーーーーーーーーーーーーーーーーー
	//デストラクタ
	EffekseerObj::~EffekseerObj() {
		DeleteEffekseerEffect(this->handleResource);
	}
	//ーーーーーーーーーーーーーーーーーーーーーーーーー
	//生成
	EffekseerObj::SP EffekseerObj::Create(const EffekseerInitData& efkData_) {
		return shared_ptr<EffekseerObj>(new EffekseerObj(efkData_));
	}
	//ーーーーーーーーーーーーーーーーーーーーーーーーー
	//再生
	std::shared_ptr<EffekseerPlayObj> EffekseerObj::Play() {
		return std::shared_ptr<EffekseerPlayObj>(new EffekseerPlayObj(PlayEffekseer3DEffect(this->handleResource)));
	}
	//ーーーーーーーーーーーーーーーーーーーーーーーーー
	//デバッグ情報
	string EffekseerObj::DebugPrint() const {
		stringstream ss;
		ss << "handle：" << this->handleResource;
		return ss.str();
	}
	//ーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーー
	//エフェクシアオブジェクト再生クラス
	//ーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーー
	EffekseerPlayObj::EffekseerPlayObj(int handle_) :
		handlePlay(handle_)
	{}
	//ーーーーーーーーーーーーーーーーーーーーーーーーー
	//座標設定
	void EffekseerPlayObj::SetPos(const ML::Vec3& pos_) {
		SetPosPlayingEffekseer3DEffect(this->handlePlay, pos_.x, pos_.y, pos_.z);
	}
	//ーーーーーーーーーーーーーーーーーーーーーーーーー
	//度数法で回転情報設定（ZXY順に回転）
	void EffekseerPlayObj::SetRotateDeg(const ML::Vec3& angleDeg_) {
		
		ML::Vec3 angRad(
			ML::ToRadian(angleDeg_.x),
			ML::ToRadian(angleDeg_.y),
			ML::ToRadian(angleDeg_.z)
			);
		SetRotationPlayingEffekseer3DEffect(this->handlePlay, angRad.x, angRad.y, angRad.z);
	}
	//ーーーーーーーーーーーーーーーーーーーーーーーーー
	//ラジアン法で回転情報設定（ZXY順に回転）
	void EffekseerPlayObj::SetRotateRad(const ML::Vec3& angleRad_) {
		SetRotationPlayingEffekseer3DEffect(this->handlePlay, angleRad_.x, angleRad_.y, angleRad_.z);
	}
	//ーーーーーーーーーーーーーーーーーーーーーーーーー
	//スケール設定（1.0が等倍）
	void EffekseerPlayObj::SetScale(const ML::Vec3& scale_) {
		SetScalePlayingEffekseer3DEffect(this->handlePlay, scale_.x, scale_.y, scale_.z);
	}
	//ーーーーーーーーーーーーーーーーーーーーーーーーー
	//色設定
	void EffekseerPlayObj::SetColor(const ML::Color& color_) {
		SetColorPlayingEffekseer3DEffect(this->handlePlay, color_.a, color_.r, color_.g, color_.b);
	}
	//ーーーーーーーーーーーーーーーーーーーーーーーーー
	//再生が終了したか
	bool EffekseerPlayObj::IsEndPlay() {
		return IsEffekseer3DEffectPlaying(this->handlePlay);
	}
	//ーーーーーーーーーーーーーーーーーーーーーーーーー
	//再生を中止　※再開不可
	void EffekseerPlayObj::Stop() {
		StopEffekseer3DEffect(this->handlePlay);
	}
	//ーーーーーーーーーーーーーーーーーーーーーーーーー
	//デバッグ情報
	string EffekseerPlayObj::DebugPrint() const{
		stringstream ss;
		ss << "handle：" << this->handlePlay;
		return ss.str();
	}
	//ーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーー
	//カメラ基本形クラス
	...
	//ーーーーーーーーーーーーーーーーーーーーーーーーー
	//描画するスクリーンの中心を返す
	ML::Vec2 Camera::getScreenCenter() {
		return Vec2(
